...
class Model_bayar extends Model
{
    ...
    public function list_nonreg($tahun)
    {
        return $this->table('program_bayar')
        ->select('program_bayar.*, nonreg_kelas.*, program.nama_program')
        ->join('nonreg_kelas', 'nonreg_kelas.nk_id = program_bayar.nonreg_kelas')
        ->join('program', 'program.program_id = nonreg_kelas.nk_program')
        ->where('bayar_tipe', 'nonreg')
        ->where('YEAR(program_bayar.tgl_bayar)', $tahun)
        ->orderBy('bayar_id', 'DESC')
        ->get()
        ->getResultArray();
    }
